...
{
  //INTERNAL_SERVER_ERROR as a default response!
  var resp = {
		statusCode: 500,
		body: "Unknown error!"
  };
  ...
  try
  {
	  ...
	  Object.assign(req.body, req.queryStringParameters);
	  ...
	  if (req.body.html)
	  {
		...
		console.info("HTML export referer: " + req.headers.referer);
		...
		catch (e)
		{
			...
			console.info("Inflate failed for HTML input: " + html);
			throw e;
		}
	  }
	  else
	  {	
		...
		else if (req.body.xmldata)
		{
			...
			{
				console.info("Inflate failed for XML input: " + req.body.xmldata);
				throw e;
			}
		}
		else
		...
		if (req.body.format && xml && req.body.w * req.body.h <= MAX_AREA)
		{
			...
			try
			{
				...
				if (req.body.format == 'png' || req.body.format == 'jpg' || req.body.format == 'jpeg')
				{
					...
					{
						console.info("Filename in request " + req.body.filename);

						headers['Content-disposition'] = 'attachment; filename="' + req.body.filename +
								'"; filename*=UTF-8\'\'' + req.body.filename;
					}
					
					headers['Content-type'] = base64encoded? 'text/plain' : ('image/' + req.body.format);
					...
					headers["content-ex-width"] = w;
					headers["content-ex-height"] = h;
					
					if (pageId != null && pageId != 'undefined')
					{
						headers["content-page-id"] = pageId;
					}

					if (scale != null && scale != 'undefined')
					{
						headers["content-scale"] = scale;
					}

					var dt = Date.now() - t0;
					
					console.info("Success " + reqStr + " dt=" + dt);
					// Cross-origin access should be allowed to now TODO Is it still needed while cors is true in config?
				...
				else if (req.body.format == 'pdf')
				{
					...
					headers['Content-type'] = base64encoded? 'text/plain' : 'application/pdf';
					headers["Content-Length"] = data.length;
				...
				else 
				{
					...
					console.warn("Unsupported Format: " + req.body.format);
				}
				await browser.close();
			}
			catch (e)
			{
				...
				console.warn("Handled exception: " + e.message
